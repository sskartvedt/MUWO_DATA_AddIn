        ...
        protected override async void OnClick()
        {
            try
            {
                ...
                else
                {
                    ...
                    // MessageBox.Show("Click 'OK' to delete the newly added fields from:" + newLine + $@"{dataSource}");
                    await
                        ExecuteDeleteFieldTool(layer, "AddedField1");
                    await
                        ExecuteDeleteFieldTool(layer, "AddedField2");
                    await
                        ExecuteDeleteFieldTool(layer, "AddedField3");
                    await
                        ExecuteDeleteFieldTool(layer, "AddedField4");
                    await
                        ExecuteDeleteFieldTool(layer, "AddedField5");
                    await
                        ExecuteDeleteFieldTool(layer, "AddedField6");
                    await
                        ExecuteDeleteFieldTool(layer, "AddedField7");
                    await
                        ExecuteDeleteFieldTool(layer, "AddedField8");
                    await
                        ExecuteDeleteFieldTool(layer, "AddedField9");
                }
            }
            catch (Exception ex)
            {
